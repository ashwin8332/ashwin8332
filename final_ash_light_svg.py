...
OUTPUT = ROOT / "final_ash_light.svg"
...
CONTRAST = 1.35                          # >1 hardens tonal separation (enhanced for clarity)
GAMMA = 0.75                             # <1 lifts midtones (brightened for light mode)
PRE_BLUR = 0.15                          # softens busy backgrounds before sampling (reduced for sharpness)
...
VIGNETTE = 0.50                          # reduced vignette for light mode (more visible portrait)
VIGNETTE_CENTER = (0.50, 0.40)           # where the face sits in the crop
VIGNETTE_INNER = 0.50                    # radius held at full brightness (increased for clarity)
...
DOT_LEVELS = 9                           # brightness buckets; bucket 0 is left blank (increased for finer detail)
DOT_SIZES = [0.0, 0.8, 1.4, 2.0, 2.6, 3.2, 3.8, 4.2, 4.6]      # dot diameter per bucket (enhanced sizes)
DOT_OPACITY = [0.0, 0.35, 0.50, 0.65, 0.75, 0.85, 0.92, 0.97, 1.0]  # improved opacity curve
...
def build_halftone(source: Path, preview: Path | None = None) -> str:
    ...
    parts.append(f'<path stroke="#0EA5E9" stroke-width="0.5" opacity="0.06" d="{lines}"/>')
    ...
    parts.append(
        f'<path stroke="#0284C7" stroke-width="1.5" opacity="0.12" d="{"".join(bars)}">'
        '<animate attributeName="opacity" values="0.12;0.03;0.14;0.05;0.12" '
...
def build_info(lines: list[str]) -> tuple[str, str, float]:
    ...
    for i, content in enumerate(lines):
        ...
        rows.append(
            f'<g clip-path="url(#lc{i})"><text x="{INFO_X}" y="0" fill="#1E293B">'
...
<defs>
  <linearGradient id="asciiGrad" x1="0%" y1="0%" x2="100%" y2="100%">
    <stop offset="0%" stop-color="#0891B2">
      <animate attributeName="stop-color" values="#0891B2;#7C3AED;#0369A1;#0891B2" dur="9s" repeatCount="indefinite"/>
    </stop>
    <stop offset="100%" stop-color="#7C3AED">
      <animate attributeName="stop-color" values="#7C3AED;#0369A1;#0891B2;#7C3AED" dur="9s" repeatCount="indefinite"/>
    </stop>
  </linearGradient>
  ...
  <linearGradient id="scanGrad" x1="0%" y1="0%" x2="0%" y2="100%">
    ...
    <stop offset="45%" stop-color="#0891B2" stop-opacity="0.05"/>
  ...
  <pattern id="scanlines" width="4" height="4" patternUnits="userSpaceOnUse">
    <rect width="4" height="1" fill="#0EA5E9" opacity="0.02"/>
  </pattern>
  ...
  <style>
    ...
    .key    {{ font-family: 'Courier New', Consolas, monospace; font-size: 15px; fill: #0891B2; font-weight: bold; }}
    ...
    .panel-title {{ font-family: 'Courier New', Consolas, monospace; font-size: 11px; fill: #0284C7; letter-spacing: 2px; opacity: 0.7; }}
    .cursor-blink {{ fill: #0891B2; }}
  </style>
</defs>
...
<g id="titlebar">
  <rect x="3" y="3" width="1174" height="34" rx="16" fill="#FFFFFF" fill-opacity="0.85"/>
...
<g transform="translate(0,38)">
  <rect x="14" y="26" width="488" height="468" rx="14" fill="#FFFFFF" fill-opacity="0.55" stroke="url(#borderGrad)" stroke-width="1" opacity="0.35"/>
  <rect x="508" y="10" width="655" height="500" rx="14" fill="#FFFFFF" fill-opacity="0.55" stroke="url(#borderGrad)" stroke-width="1" opacity="0.35"/>
  ...
  <g stroke="#0284C7" stroke-width="1" opacity="0.35" fill="none">
